...
frame1=Frame(root,width=1080,height=720,relief=RIDGE,borderwidth=5,bg='#2ef8e5').place(x=0,y=0)
...
def listendata():
    ...
    if(cur>0 and cur<12):
        engine.say("good morning sir, welcome to webscraping project. please tell me the item name,so that i can scrappe the website and produce the resut.")
    elif(cur>12 and cur<16):
        engine.say("good afternoon sir, welcoe to web scrapping project. please tell me the item name,so that i can scrappe the website and produce the resut.")
    else:
        engine.say("good evening sir , welcome to web scrapping project. please tell me the item name,so that i can scrappe the website and produce the resut.")

    engine.runAndWait()
    lisdata=listen()
    product_name1.delete(1.0,'end')
    product_name1.insert('end',lisdata)
    

product_name1=Text(root,width=10,height=0,borderwidth=1,relief=RIDGE,font=('verdana',15))
product_name1.place(x=500,y=360)
product_name2=Button(text="say",width=20,height=0,command=listendata).place(x=660,y=360)
...
def start():
    ...
    #print(rate[0].text)
        ...
        pd.DataFrame(records_list,columns=['model','star','number of ratings','number of reviews','system apps','operating sysytem','display','speaker','refresh rate','modes','warranty','price'])

        with open('Flipkart_results.csv','w',newline='',encoding='utf-8') as f:
            writer=csv.writer(f)
            writer.writerow(['model','star','number of ratings','number of reviews','system apps','operating sysytem','display','speaker','refresh rate','modes','warranty','price'])
            writer.writerows(records_list)
        scraped=pd.read_csv("Flipkart_results.csv")
        print(scraped.tail())

def openf():
    os.startfile( "E:\\node_js\\Flipkart_results.csv" )

enterbut=Button(text="Enter",width=20,height=0,command=start)
enterbut.place(x=550,y=400)
enterbut=Button(text="Open File",width=20,height=0,command=openf).place(x=550,y=500)
